...
| **Persistence** | Use WorkflowCore's EF tables vs custom tables | Custom `IPersistenceProvider` | WorkflowCore's EF provider pins EF ≤7.x — incompatible with Umbraco CMS 18 (EF 10.x). Custom implementation runs on our own `UmbracoAutomateDbContext` (EF 10), adopting WorkflowCore's normalized pointer schema; avoids the version clash. |
