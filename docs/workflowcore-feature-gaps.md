...
> **Implemented** as `ForEachContainerStepBody` (custom — see [control-flow-architecture.md](control-flow-architecture.md)). Performance of this body was subsequently hardened in PR #128 (collection materialised once per loop; iteration state kept out of the persisted blob). Original gap analysis retained below for context.
