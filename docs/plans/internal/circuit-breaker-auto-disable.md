...
- **Per-initiator policy.** "Allowed while disabled" should match **human-initiated** runs. The breaker uses an `IsInteractive(initiatorType)` predicate matching `TriggerInitiatorType.User` (manual "run now") and `TriggerInitiatorType.Replay` (replay); `System` / `Scheduled` / `Webhook` are blocked. Gated by `AllowManualRunWhileDisabled`. The run is a **pure test**: success does **not** close the circuit — re-instatement is always explicit (see [1.6](#16-reset)).
...
Two cleanups surfaced during this review have since shipped, and the plan above assumes them:

- **[#63](https://github.com/umbraco/Umbraco.Automate/issues/63)** — `TriggerInitiatorType.Replay` constant added; `ReplayRunController` now passes it (`ReplayRunController.cs:94`). The breaker's `IsInteractive` predicate matches `User` + `Replay`.
- **[#64](https://github.com/umbraco/Umbraco.Automate/issues/64)** — `AutomationStatus.Inactive` renamed to `Unpublished`.
