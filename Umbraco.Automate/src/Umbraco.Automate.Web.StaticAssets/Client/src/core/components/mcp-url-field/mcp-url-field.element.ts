...
/**
 * Read-only display of an automation's MCP endpoint URL, with a copy button.
 * Shared by the trigger settings sidebar and the automation Info view so the two
 * can't drift apart. Mirrors {@link UaWebhookUrlFieldElement}.
 */
@customElement("ua-mcp-url-field")
export class UaMcpUrlFieldElement extends UmbLitElement {
    /** Unique of the automation whose MCP URL should be shown. */
    ...
    override render() {
        return html`
            ...
            <uui-button
                ...
                look="secondary"
                ?disabled=${!this._url}
                label=${this.localize.term("uaMcp_copyUrl")}
    ...
    async #loadUrl() {
        ...
        const { data, error } = await tryExecute(
            this,
            AutomationsService.getAutomationsByIdMcpUrl({ path: { id: this.automationId } }),
        );

        if (error) {
            const notifications = await this.getContext(UMB_NOTIFICATION_CONTEXT);
            notifications?.peek("danger", { data: { message: this.localize.term("uaMcp_urlLoadFailed") } });
        }
    ...
    async #copy(url: string) {
        ...
        try {
            ...
            notifications?.peek("positive", {
                data: { message: this.localize.term("uaMcp_urlCopied") },
            });
        } catch {
            notifications?.peek("danger", {
                data: { message: this.localize.term("uaMcp_urlCopyFailed") },
            });
        }
    }
...
export default UaMcpUrlFieldElement;

declare global {
    interface HTMLElementTagNameMap {
        "ua-mcp-url-field": UaMcpUrlFieldElement;
    }
}
