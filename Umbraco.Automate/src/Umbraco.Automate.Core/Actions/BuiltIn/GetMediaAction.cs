...
public sealed class GetMediaAction : ActionBase<GetMediaSettings, GetMediaOutput>
{
    ...
    // IPublishedContent has no Parent property — query the navigation structure directly
    // rather than materialising the parent media item.
    private Guid? ResolveParentKey(IPublishedContent media)
        => _navigationQueryService.TryGetParentKey(media.Key, out Guid? parentKey) ? parentKey : null;
    ...
    private GetMediaOutput Project(IPublishedContent media, string? culture, Guid? creatorKey, Guid? writerKey)
        => new()
        {
            ...
            ParentKey = ResolveParentKey(media),
            Level = media.Level,
            Path = media.Path,
