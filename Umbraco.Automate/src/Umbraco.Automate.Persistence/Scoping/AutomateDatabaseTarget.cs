...
internal static class AutomateDatabaseTarget
{
    ...
    /// <summary>
    /// Collapses the spellings SQL Server treats as the local machine so that, for example,
    /// <c>(local)</c> and <c>.</c> are recognised as the same server.
    /// </summary>
    private static string Normalise(string server)
    {
        var trimmed = server.Trim();

        return trimmed is "." or "(local)" or "localhost"
            ? "localhost"
            : trimmed;
    }
